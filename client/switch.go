...
func (s Switch) create() func(string) error {
	return func(cmd string) error {
		...
		if err != nil {
			return wrappError("Could not create reminder", err)
		}
...
func (s Switch) edit() func(string) error {
	return func(cmd string) error {
		...
		if err != nil {
			return wrappError("Could not edit reminder", err)
		}
...
func (s Switch) fetch() func(string) error {
	return func(cmd string) error {
		...
		if err != nil {
			return wrappError("Could not fetch reminder(s)", err)
		}
...
func (s Switch) delete() func(string) error {
	return func(cmd string) error {
		...
		if err != nil {
			return wrappError("Could not delete reminder(s)", err)
		}
		fmt.Printf("Reminder deleted record(s) successfully: \n%v\n", ids)
		return nil
	}
}
...
func (s Switch) parseCmd(cmd *flag.FlagSet) error {
	...
	if err != nil {
		return wrappError("Could not parse '"+cmd.Name()+"' command flags", err)
	}
	return nil
}
...
func (s Switch) checkArgs(minArgs int) error {
	...
	if len(os.Args)-2 < minArgs {
		...
		return fmt.Errorf("%s expects at least %d arg(s), %d provided",
			os.Args[1],
			minArgs,
			len(os.Args)-2,
		)
